...
bool SQL_CUSTOM::init(AbstractExt *extension, const std::string &database_id, const std::string &options_str)
{
	...
	{ 
		//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: No Database Connection: {0}", database_id);
		return false;
	}
	...
	{ 
		//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: Missing Config Parameter");
		return false;
	}
	...
	try
	{
		...
		if (boost::filesystem::exists(custom_ini_path))
		{
			if (boost::filesystem::is_regular_file(custom_ini_path))
			{
				...
				//extension_ptr->logger->info("extDB4: SQL_CUSTOM: Loading Template Error: Not Regular File: {0}", custom_ini_path.string());
				return false;
			}
		} else { 
			//extension_ptr->logger->info("extDB4: SQL_CUSTOM: {0} doesn't exist", custom_ini_path.string());
			return false;
		}
	}
	catch (boost::filesystem::filesystem_error &e)
	{ 
		//extension_ptr->logger->info("extDB4: SQL_CUSTOM: filesystem_error: {0}", e.what());
		return false;
	}
}
...
bool SQL_CUSTOM::loadConfig(boost::filesystem::path &config_path)
{
	...
	try
	{
		...
		for (auto& value : ptree.get_child("Default")) {
			//extension_ptr->logger->info("extDB4: SQL_CUSTOM Config Error: Section Default Unknown Setting: {0}", value.first);
			status = false;
		}
		...
		for (auto& section : ptree) {
					...
					for (auto &token : tokens)
					{
						...
						for (auto &sub_token : sub_tokens)
						{
							...
							else
							{
								...
								{
									//extension_ptr->logger->info("extDB4: SQL_CUSTOM Config Error: Section: {0} Unknown INPUT Option: {1} in {2}", section.first, sub_token, token);
									status = false;
								}
							}
						}
					...
					for (auto &token : tokens)
					{
						...
						for (auto &sub_token : sub_tokens)
						{
							...
							else
							{
								...
								{
									#ifdef DEBUG_TESTING
										extension_ptr->console->info("extDB4: SQL_CUSTOM Config Error: Section: {0} Unknown OUTPUT Option: {1} in {2}", section.first, sub_token, token);

									#endif
									//extension_ptr->logger->info("extDB4: SQL_CUSTOM Config Error: Section: {0} Unknown OUTPUT Option: {1} in {2}", section.first, sub_token, token);
									//extension_ptr->logger->info("extDB4: SQL_CUSTOM Config Error: Debug: {0}", output_options_str);
									status = false;
								}
							}
						}
			...
			for (auto& value : section.second) {
				//extension_ptr->logger->info("extDB4: SQL_CUSTOM Config Error: Section: {0} Unknown Setting: {1}", section.first, value.first);
				status = false;
			}
		}
		return status;
	...
	{
		//extension_ptr->logger->info("extDB4: SQL_CUSTOM Config Error: {0}", e.what());
		return false;
	}
...
bool SQL_CUSTOM::query(std::string &input_str, std::string &result, std::vector<std::vector<std::string>> &result_vec, std::vector<std::string> &tokens, MariaDBSession &session, std::string &insertID, std::unordered_map<std::string, call_struct>::iterator &calls_itr)
{
	...
	for (auto &sql : calls_itr->second.sql)
	{
		...
		for (int i = 0; i < sql.input_options.size(); ++i)
		{
			...
			if (sql.input_options[i].strip)
			{
				...
				if (stripped_str != tmp_str)
				{
					...
					{
						case 2: // Log + Error
							//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: Error Bad Char Detected: Input: {0} Token: {1}", input_str, tmp_str);
							result = "[0,\"Error Strip Char Found\"]";
							return false;
						case 1: // Log
							break;//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: Error Bad Char Detected: Input: {0} Token: {1}", input_str, tmp_str);
					}
		...
		{
			//extension_ptr->logger->error("extDB4: SQL: Error MariaDBQueryException: {0}", e.what());
			//extension_ptr->logger->error("extDB4: SQL: Error MariaDBQueryException: Input: {0}", input_str);
...
bool SQL_CUSTOM::preparedStatementPrepare(std::string &input_str, std::string &result, std::vector<std::vector<std::string>> &result_vec, MariaDBSession &session, MariaDBStatement *session_statement_itr, std::string callname, std::unordered_map<std::string, call_struct>::iterator &calls_itr)
{
	...
	{
		//extension_ptr->logger->error("extDB4: SQL: Error MariaDBStatementException0: {0}", e.what());
		//extension_ptr->logger->error("extDB4: SQL: Error MariaDBStatementException0: Input: {0}", input_str);
	...
	{
		//extension_ptr->logger->error("extDB4: SQL: Error MariaDBStatementException1: {0}", e.what());
		//extension_ptr->logger->error("extDB4: SQL: Error MariaDBStatementException1: Input: {0}", input_str);
		result = "[0,\"Error MariaDBStatementException1 Exception\"]";
		session.resetSession();
		return false;
	}
	catch (extDB4Exception &e)
	{
		//extension_ptr->logger->error("extDB4: SQL: Error extDB4Exception: {0}", e.what());
		//extension_ptr->logger->error("extDB4: SQL: Error extDB4Exception: Input: {0}", input_str);
		result = "[0,\"Error extDB4Exception Exception\"]";
		session.resetSession();
		return false;
	}
	return true;
		...
		for (int i = 0; i < processed_inputs.size(); ++i)
		{
			...
			if (calls_itr->second.sql[sql_index].input_options[i].strip)
			{
				...
				if (stripped_str != processed_inputs[i].buffer)
				{
					...
					{
						case 2: // Log + Error
							//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: Error Bad Char Detected: Input: {0} Token: {1}", input_str, processed_inputs[i].buffer);
							result = "[0,\"Error Strip Char Found\"]";
							return true;
						case 1: // Log
							//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: Error Bad Char Detected: Input: {0} Token: {1}", input_str, processed_inputs[i].buffer);
...
bool SQL_CUSTOM::callProtocol (std::string input_str, std::string &result, const bool async_method, const unsigned int unique_id)
{
	...
	if (calls_itr == calls.end())
	{
		...
		//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: Error No Custom Call Not Found: Input String {0}", input_str);
		//extension_ptr->logger->warn("extDB4: SQL_CUSTOM: Error No Custom Call Not Found: Callname {0}", callname);
	...
	try
	{
		...
		{
			throw extDB4Exception("Config Invalid Number Number of Inputs Got " + std::to_string(tokens.size()-1) + " Expected " + std::to_string(calls_itr->second.highest_input_value));
		}
		...
		if (!success)
		{
			//extension_ptr->logger->error("extDB4: SQL: Error Max Retrys Reached");
			//extension_ptr->logger->error("extDB4: SQL: Error Max Retrys Reached");
			return true;
		}
		result = "[1,[";
	...
	catch (extDB4Exception &e) // Make new exception & renamed it
	{
		//extension_ptr->logger->error("extDB4: SQL: Error extDB4Exception: {0}", e.what());
		//extension_ptr->logger->error("extDB4: SQL: Error extDB4Exception: Input: {0}", input_str);
		result = "[0,\"Error extDB4Exception Exception\"]";
	...
	{
		//extension_ptr->logger->error("extDB4: SQL: Error MariaDBConnectorException: {0}", e.what());
		//extension_ptr->logger->error("extDB4: SQL: Error MariaDBConnectorException: Input: {0}", input_str);

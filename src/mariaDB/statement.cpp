...
void MariaDBStatement::bindParams(std::vector<MariaDBStatement::mysql_bind_param> &params)
{
	...
	{
		throw extDB4Exception("SQL Invalid Number of Inputs Got " + std::to_string(params.size()) + " Expected " + std::to_string(params_count));
	}
	...
	for (int i=0 ; i<params_count ; i++)
	{
		...
		switch (param.type)
		{
						...
						try
						{
							...
							switch (i)
							{
								...
								case 5:
									...
									break;
								default:
									throw extDB4Exception("Invalid Time Format1: [" + param.buffer + "]");
								}
						}
						catch(std::exception const &e)
						{
							throw extDB4Exception("Invalid Time Format2: [" + param.buffer + "]");
						}
					}
				} else {
					throw extDB4Exception("Invalid Time Format3: " + param.buffer);
				}
			...
			{
				throw extDB4Exception("Field Type not supported: LONGBLOB/LONGTEXT");
			}
			default:
				throw extDB4Exception("Unknown Field Type: " + std::to_string(param.type));
		}
...
void MariaDBStatement::execute(std::vector<sql_option> &output_options, std::string &strip_chars, int &strip_chars_mode, std::string &insertID, std::vector<std::vector<std::string>> &results)
{
	...
	if (mysql_stmt_result_metadata_ptr)
	{
		...
		for (unsigned int i = 0; i < num_fields; i++)
		{
			...
			case MYSQL_TYPE_LONG_BLOB:
			{
				throw extDB4Exception("MYSQL_TYPE_LONG_BLOB type not supported when using Prepared Statements");
			}
			case MYSQL_TYPE_NEWDECIMAL:
			case MYSQL_TYPE_DECIMAL:
			case MYSQL_TYPE_STRING:
			...
			for (unsigned int i = 0; i < num_fields; i++)
			{
				if (bind_data[i].isNull)
				{
					...
					switch (fields[i].type)
					{
						...
						case MYSQL_TYPE_LONG_BLOB:
						{
							throw extDB4Exception("MYSQL_TYPE_LONG_BLOB type not supported");
						}
						default:
							std::string tmp_str;
							...
							if (output_options[i].strip)
							{
								...
								if (stripped_str != tmp_str)
								{
									...
									{
									case 2: // Log + Error
										throw extDB4Exception("Bad Character detected from database query");
										//case 1: // Log
										//logger->warn("extDB4: SQL_CUSTOM: Error Bad Char Detected: Input: {0} Token: {1}", input_str, processed_inputs[i].buffer);
									}
